#!/usr/bin/env bash
#
# Test rápido (sin red) de resolve_user_zshrc en setup_linux.sh.
#
...
#
# Uso:  bash apps/tests/test_terminal_detection_linux.sh
...
passed=0; failed=0
THOME="/tmp/zshrc-detect-home-linux"   # HOME simulado y determinista
...
printf "%s=== Test: resolución de .zshrc por terminal — Linux ===%s\n" "$CYAN" "$NC"
printf "%s(HOME simulado = %s)%s\n\n" "$CYAN" "$THOME" "$NC"

# --- Clase A: terminales que NO tocan $ZDOTDIR -> ~/.zshrc ---
check "GNOME Terminal / Konsole / Kitty (sin ZDOTDIR)" "$THOME/.zshrc"
