#!/usr/bin/env bash
#
# Test AISLADO del flujo SSH multi-cuenta de setup_linux.sh.
#
...
#
# Uso:  bash apps/tests/test_ssh_multiaccount_linux.sh
# Exit: 0 si todo pasa.
...
SANDBOX="$(mktemp -d /tmp/ssh-linux.XXXXXX)"
...
printf "%s=== Test: SSH multi-cuenta — Linux (gh/ssh mockeados) ===%s\n" "$CYAN" "$NC"
printf "%sHOME=%s%s\n\n" "$CYAN" "$SANDBOX" "$NC"
...
[ "$(grep -cE '^Host \*$'               "$CFG")" = "1" ]; ok $? "Bloque 'Host *' (defaults) presente una sola vez"
...
# --- asserts: 'Host *' SIN UseKeychain (opción exclusiva de macOS) ----------
! grep -q 'UseKeychain' "$CFG"; ok $? "No se escribe 'UseKeychain' (correcto en Linux)"
grep -q 'AddKeysToAgent yes' "$CFG"; ok $? "Se escribe 'AddKeysToAgent yes'"
...
# clean_ssh_config usa `sed -i` (sintaxis GNU). Solo es válido bajo GNU sed;
# en un host con BSD sed (macOS) se omite con aviso.
if sed --version >/dev/null 2>&1; then
    ...
    printf "%s⚠ clean_ssh_config: omitido (requiere GNU sed; este host usa BSD sed)%s\n" "$YELLOW" "$NC"
fi
