...
echo "${CYAN}=== Smoke Test — Linux Setup ===${NC}"
echo

echo "${CYAN}--- Herramientas base ---${NC}"
PKG_MGR="$(command -v apt-get || command -v dnf || command -v pacman 2>/dev/null)"
assert "[ -n '$PKG_MGR' ]"  "Gestor de paquetes disponible"  "$PKG_MGR"
assert "command -v git"     "Git instalado"                   "$(command -v git 2>/dev/null)"
assert "command -v zsh"     "Zsh instalado"                   "$(command -v zsh 2>/dev/null)"
...
assert "[ \$(stat -c '%a' $HOME/.ssh/config) = '600' ]"     "Permisos correctos en ~/.ssh/config"   "$(stat -c '%a' $HOME/.ssh/config 2>/dev/null)"
