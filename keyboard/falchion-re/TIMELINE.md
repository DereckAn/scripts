...
The 63 policy entries begin at RAM `0x1801c810`. Their values and initializer
have not yet been recovered. Related runtime tables include:
...
| `CandidateB_Entry` label | Corrected to provisional `CandidateB_Start_Function` |
...
- solved Candidate B's integrity field or runtime table initialization;
- built or flashed custom firmware.

## Recommended continuation

The safest high-value continuation is offline tracing of how Candidate B
initializes RAM `0x1801bff6` and `0x1801c810`. Recovering those tables would
clarify target encoding and the exact unsupported-key set without touching the
keyboard.
